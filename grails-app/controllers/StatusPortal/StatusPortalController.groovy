...
@Secured('permitAll')
		...
		try{
			...
			if(presentTicket){
				...
				if(ticketSummaryObject){
					newTicketStatus.ticket=TicketSummary.findByTicket_id(data.ticketData.ticket_id)
				}else{
					...
					render flash.message="Successfully data saved "
			...
			redirect controller:'StatusPortal',action:'index1'
	...
	//for fetching the data on the basis of ticket-id
	@Secured('permitAll')
	def ticketHistory(){
